...
# Enable silent fallback to see cleaner test results on Jenkins
export MPIR_CVAR_COLLECTIVE_FALLBACK=silent
if [ ${algorithm} == "k_brucks" ] || [ ${algorithm} == "ring" ] || [ ${algorithm} == "tree" ] || [ ${algorithm} == "recexch" ] || [ ${algorithm} == "recexch_doubling" ] || [ ${algorithm} == "recexch_halving" ]; then
    export MPIR_CVAR_${collective}_DEVICE_COLLECTIVE=false
    export MPIR_CVAR_${collective}_INTRA_ALGORITHM=${algorithm}
elif [ ${algorithm} == "trigger_tree_tagged" ] || [ ${algorithm} == "trigger_tree_rma" ] || [ ${algorithm} == "trigger_tree_pipelined" ] || [ ${algorithm} == "trigger_recexch" ]; then
    if [ ${collective} == "BCAST" ] || [ ${collective} == "ALLREDUCE" ]; then
        if [ ${collective} == "BCAST" ]; then
            export MPIR_CVAR_I${collective}_COMPOSITION=3
        elif [ ${collective} == "ALLREDUCE" ]; then
            export MPIR_CVAR_I${collective}_COMPOSITION=2
        fi
        export MPIR_CVAR_CH4_OFI_ENABLE_TRIGGERED=1
        export MPIR_CVAR_${collective}_DEVICE_COLLECTIVE=false
        export MPIR_CVAR_${collective}_INTRA_ALGORITHM=nb
        export MPIR_CVAR_I${collective}_OFI_INTRA_ALGORITHM=${algorithm}
    else
        export MPIR_CVAR_CH4_OFI_ENABLE_TRIGGERED=1
        export MPIR_CVAR_${collective}_OFI_INTRA_ALGORITHM=${algorithm}
    fi
elif [${algorithm} == "trigger_tree_small_message" ]; then
    if [ ${collective} == "BCAST" ] || [ ${collective} == "ALLREDUCE" ]; then
	export MPIR_CVAR_CH4_OFI_ENABLE_TRIGGERED=1
        export MPIR_CVAR_${collective}_OFI_INTRA_ALGORITHM=${algorithm}
    fi
    ...
    export MPIR_CVAR_${collective}_DEVICE_COLLECTIVE=false
    export MPIR_CVAR_I${collective}_DEVICE_COLLECTIVE=false
    export MPIR_CVAR_${collective}_INTRA_ALGORITHM=nb
    export MPIR_CVAR_I${collective}_INTRA_ALGORITHM=${algorithm}
fi
